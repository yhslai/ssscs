...
object Ssscs {
  ...
  def main(args: Array[String]) {
    val defaultConfig = CrawlConfig(200)
    val infos = crawlInfos(defaultConfig)

    println(infos.valueTreeString)
  }

  private def crawlInfos(config: CrawlConfig): Vector[ArticleInfo] = {
    ...
    def filterEffectiv(infos: Vector[ArticleInfo]): Vector[ArticleInfo] = {
    ...
    val iterator = iterStarter(infos => {
      filterEffectiv(infos ++ crawlIter(infos.size + 1))
    })
